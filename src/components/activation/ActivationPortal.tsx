...
type Step = "code" | "verified" | "credentials" | "progress" | "completed";

/**
 * Customer-facing activation portal. It has no link to, and no knowledge of,
 * the admin dashboard, and it only ever talks to the public activation API —
 * which never returns card data or any other customer's activation.
 */
export function ActivationPortal({
  ...
  useEffect(() => {
    if (step !== "progress" || !session) return;
    let cancelled = false;

    async function poll() {
      try {
        const result = await activationProgress({ data: { code, session } });
        if (cancelled) return;
        setState(result.state);
        setStage(result.stage);
        setProgress(Math.max(4, result.progress));
        if (result.state === "done") setStep("completed");
      } catch {
  ...
  async function submitCode(event: React.FormEvent) {
    ...
    setError("");
    try {
      const result = await verifyActivationCode({ data: { code, prefix: prefix ?? null } });
      if (!result.valid) {
  ...
  async function submitCredentials(event: React.FormEvent) {
    ...
    setError("");
    try {
      const result = await beginActivation({ data: { code, email, password } });
      setPassword("");
      ...
      <div className="relative w-full max-w-md">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="mt-4 text-2xl font-semibold tracking-tight">
            {brandLabel ? `Activate ${brandLabel}` : "Activate your subscription"}
          </h1>
          <p className="mt-2 max-w-sm text-sm text-muted-foreground">
            A few quick steps. We take care of the payment and the renewal for you.
          </p>
        </div>
        ...
        <section className="panel sheen animate-rise mt-6 p-7">
          ...
          {step === "completed" ? (
            <div className="space-y-6 py-2 text-center">
              <BadgeCheck className="mx-auto size-11 text-success" />
              <div>
                <h2 className="text-lg font-semibold">Activation completed</h2>
              ...
              <div className="rounded-xl border border-border bg-surface-raised px-5 py-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  A confirmation is also visible in your own account.
                </p>
              </div>
            </div>
          ) : null}
...
function StepDots({ step }: { step: Step }) {
  const index =
    step === "code" ? 0 : step === "progress" ? 2 : step === "completed" ? 3 : 1;
  const labels = ["Code", "Account", "Activation", "Done"];
  return (
